...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DueCoinTEST2\r");
            Send(hSocket, "WHO #DueCoinTEST2\r");
        } else {
            // randomly join #DueCoin00-#DueCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #DueCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #DueCoin%02d\r", channel_number).c_str());
        }
