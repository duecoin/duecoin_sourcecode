DueCoin-qt: Qt4 GUI for DueCoin
===============================
...
Alternatively, install Qt Creator and open the `DueCoin-qt.pro` file.

An executable named `DueCoin-qt` will be built.
...
A warning for people using the *static binary* version of DueCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of DueCoin is linked against libdb5.3.
...
installed causes DueCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
